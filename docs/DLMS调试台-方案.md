...
> ㉜ 对外部审核报告 v2 逐条核对 → **§18**（含一处否定性结论被误判的方法论）；
>
> **v1.6 变更**（安全时序 + 可观测性）：
...
> 本章原为对一份**外部 AI 生成的审核报告**（16 项结论）的逐条核验记录。
> 该报告为一次性产物，**已并入本章后删除** —— 保留下的是核验结论与处置，
> 全部可凭**源码位置 / 提交号 / §12 风险编号**在仓库内复核，不依赖外部文件。

核验方式：逐条到源码复核（**不照单全收**）。结论分三类：
...
—— 这正是原审核报告的 **D5**（§15），当时标为"确认但转用户决定、未改"。
...
## 18. 外部审核问题处置记录（2026-09-24）

> 本章原为对一份**外部 AI 生成的审核报告**的逐条核对（该报告为一次性产物，**已并入本章后删除**）。
> 保留下来的只有**核验结论与处置**，且全部可凭仓库内证据复核 ——
> 提交号（`git show`）、风险编号（§12）、代码位置，不依赖任何外部文件。
...
| 问题 | 处置 | 证据 |
|---|---|---|
| **D1** 重发前未重置接收游标 | 重发前重置 `rx.size/position + complete` | `DLMSBridge.c:174-176` |
| **D2** 重试守卫失效 | 收到数据时 `fail = 0` | `DLMSBridge.c:~197` |
| **D3** 整数窄化/符号扩展 | `(uint64_t)(uint32_t)v->lVal` + 构造侧 `uint32_t u` | `DLMSBridge.c:722 / 1104` |
| **D4** 输出缓冲溢出 | `outBufferSize = 4096` + 截断提示 | `GXDLMSReader.swift:132` |
| **D5** 超时后晚到字节丢失 | 传输层加**预读缓冲** + `NSLock` | `GXDLMSTransport.swift` |
| **D6** 连接未释放 | `conn?.cancel()` | `GXDLMSTransport.swift:32-60` |
| **P1** 日志无上限 | 高水位 2500 → `suffix(2000)` | `Store.swift` |
| **P2** 并发写盘 | 串行 `saveQueue` + `.atomic` | `Store.swift` |
| **M1** 密钥参数命名误导 | 改为 `blockCipherKeyHex / authenticationKeyHex / dedicatedKeyHex` | `DLMSCore.h` |

**新发现并已修** ✓（详见 §12 风险登记）：

| 问题 | 处置 | 风险编号 |
|---|---|---|
| **N1** 预读缓冲栈溢出 | `takePending(_:max:)` 按 max 截断、余量留 pending；回调侧 `min(cap, count)` 兜底 | R29 |
| **N2** `pending` 无上限 | `pendingLimit = 256KB`（对齐 C 侧 `DLMS_MAX_RX_BYTES`），超限丢**最旧** | R29 |
| **N3** `parseEntries` 裁剪口径不一致 | 改用与 `logs` 相同的高水位裁剪（`parseHighWater = 250`） | — |
| **N4** 死代码 | 删 `private enum Kind` 与 `check(_:step:)`（grep 确认只有定义、无调用） | — |
| **N5** 静默吞错 | `Store.load` 区分「文件不存在」（首次运行，静默）与「**存在但解析失败**」（记 warn）；写盘失败仍静默，见 §13.2 | — |
| **R30** `selectObis` 用**原文**匹配清单 | 改用 `ObisUtil.comparisonKey` 归一后比较 | R30 |

提交：`a71adba`（N1–N5）· `c6d1ff8`（R26 根因）· 同批（R30）。

### 18.2 一处被误判的结论（方法论）

那份报告把 **S2「导入文件无大小上限」标成「❌ 未改」** —— **实际早已修**：
...
### 18.3 N1 的严重性被低估（本轮补充发现）

报告称触发条件是「连续两次 recv 超时 → `pending` 累积 4096 → 写爆 2048 栈缓冲」。
**实际更严重**：每次 `receive` 都新起一个 completion，而**超时后没有任何取消机制**
（全文件唯一的 cancel 是连接级的 `conn?.cancel()`，Network.framework 没有"取消单个 receive"的 API）
→ C 层每超时重试一次就多留一个挂起 completion，而 C 层允许重试到
`DLMS_MAX_RECV_ROUNDS = 256`（`DLMSBridge.c:120`）
→ 同一时刻可挂起 N 个 completion，对端一次发来长数据时**接连触发**、各返回 ≤2048 字节
并全部 append 进 `pending` → 可达 **256 × 2048 = 512KB** ✗（而非报告说的 2 × 2048）。

且 `dlmsTransportRecv` 是 `d.copyBytes(to: buf, count: d.count)` ——
`copyBytes` 是 `UnsafeMutablePointer` 的**无边界检查写入** → 这是**确定的栈破坏**，
不只是"溢出风险"。

**更根本的修法是常驻接收循环**（一次只挂一个 completion，收到就入队、永不因超时丢弃），
这也是 Network.framework 的惯用法 —— 已记入 §13.2 待排期。

### 18.4 未纳入（属"新增能力/加固"，非"已核实的问题"）

- **M2** 报文类型启发式 → 精确化需 bridge 回传 `gxReplyData.command`（已文档化 §8 + R20）
- **M5** `ObisImporter` 单测缺口（含 `parseBare` 不处理 RFC4180 转义引号 `""`）
- **S1** 密钥入 Keychain（明文在 `config.json`，备份可导出；自用调试台，改动面大）
- mock Set-Response 保真度
